...
function baixarPdfUltimaFrequencia() {
  ...
  const dataFormatada = formatDateBR(dataUltimaFrequencia);
  const linhas = registrosUltimaFrequencia.map(r => [
    r.nome,
  ...
  doc.text(`Última frequência: ${dataFormatada}`, 40, 64);
  doc.text(`Total de assinaturas: ${registrosUltimaFrequencia.length}`, 40, 80);
  ...
  doc.save(`frequencia-${normalizarNomeArquivo(dataFormatada)}.pdf`);
}
